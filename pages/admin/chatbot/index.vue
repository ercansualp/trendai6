            ...
            >
              <td class="px-6 py-4 whitespace-nowrap text-sm text-foreground flex items-center">
                <img
                  ...
                  @click="openImageViewModal(getUserAvatar(session.userId))"
                />
                <span>{{ getUserFullName(session.userId) }}</span>
              </td>
...
const fetchSessions = async () => {
  ...
  try {
    ...
    // Ensure mockSessions are initially in a consistent order if no sorting is applied by default
    sessions.value = [...mockSessions].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    showNotification('Chat sessions loaded successfully!', 'success')
  } catch (error) {
